...
import {
  ...
  isRoomCountValid,
  selectedRoomNumbers,
} from './roomRegistration.mjs';

test('generates room numbers by floor and unit number', () => {
  assert.deepEqual(generateRoomNumbers(2, 3), [
    '101',
    '102',
    '103',
    '201',
...
test('bulk request includes only rooms explicitly selected in the preview', () => {
  assert.deepEqual(
    selectedRoomNumbers(['101', '102', '201'], new Set(['101', '201'])),
    ['101', '201'],
  );
});
...
test('groups two-digit floor numbers without matching floors that share a prefix', () => {
  ...
  assert.deepEqual(floors[9], {
    floor: 10,
    roomNumbers: ['1001', '1002', '1003'],
  });
