  ...
  if (!reissuePromise) {
    reissuePromise = fetch(`${API_BASE}/auth/reissue`, {
      ...
      .then(async (response) => {
        ...
        const payload = (await response.json()) as DataResponse<{ accessToken: string }>;
...
async function send(path: string, { method = "GET", body, query, auth = true, signal }: RequestOptions) {
  ...
  try {
    return await fetch(`${API_BASE}${path}${toQueryString(query)}`, {
      method,
      headers,
