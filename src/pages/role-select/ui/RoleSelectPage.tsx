  ...
  return (
    <AuthLayout>
      ...
      <RadioGroup
        aria-label="사용자 역할"
        value={selectedRole}
        onValueChange={(value) => setSelectedRole(value as RouteRole)}
      >
...
function roleSelectionErrorMessage(error: unknown) {
  ...
  if (error.status === 409)
    return '역할은 최초 한 번만 설정할 수 있어요. 이미 역할이 설정된 계정인지 확인해 주세요.';
  if (error.status === 401)
    return '로그인 정보가 만료됐어요. 다시 로그인해 주세요.';
  if (error.status === 0) return error.message;
  if (error.status === 400 || error.status === 422)
    return '역할 선택 정보를 확인해 주세요.';
  return '역할을 저장하지 못했어요. 잠시 후 다시 시도해 주세요.';
}
