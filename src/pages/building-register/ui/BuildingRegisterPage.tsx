...
function loadAddressSearchScript() {
  ...
  addressSearchScript = new Promise((resolve, reject) => {
    ...
    script.onerror = () => { addressSearchScript = null; reject(new Error("주소 검색 스크립트를 불러오지 못했습니다.")); };
...
declare global {
  interface Window {
    daum?: {
      Postcode: new (options: { oncomplete: (result: { roadAddress: string; jibunAddress: string }) => void }) => { open: () => void };
    };
  }
}
