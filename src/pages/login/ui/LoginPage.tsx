...
export function LoginPage() {
  ...
  const [email, setEmail] = useState(
    signupComplete ? (signupState?.email ?? '') : '',
  );
  ...
  return (
    <AuthLayout>
      ...
      {signupComplete && (
        <Callout
          tone="positive"
          description="회원가입이 완료됐어요. 가입한 이메일로 로그인해 주세요."
        />
      )}
      <form className="form-stack" onSubmit={submit} noValidate>
        ...
        {error && error.violations.length === 0 && (
          <Callout
            tone="critical"
            description={loginErrorMessage(error, countdown.remaining)}
          />
        )}
        <ActionButton
          type="submit"
      ...
      <p className="auth-footer">
        아직 계정이 없나요? <Link to="/auth/signup">회원가입</Link>
      </p>
    </AuthLayout>
  );
}
...
function loginErrorMessage(error: ApiError, remainingSeconds: number) {
  ...
  if (error.status === 429)
    return `요청이 너무 많아요. ${remainingSeconds}초 후 다시 시도해 주세요.`;
