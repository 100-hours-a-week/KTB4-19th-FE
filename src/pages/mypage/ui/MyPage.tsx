      ...
      <div className="detail-grid">
        <section className="panel profile-panel">
          <div className="profile-head">
            ...
            <div>
              <h2>{role === 'manager' ? '김관리' : '박입주'}</h2>
              <p>{role === 'manager' ? '관리자' : '입주민'}</p>
            </div>
            <ActionButton variant="neutralOutline">프로필 수정</ActionButton>
          </div>
          <InfoRow
            label="이메일"
            value={role === 'manager' ? 'manager@zips.ai' : 'resident@zips.ai'}
          />
          <InfoRow
            label="연락처"
            value={role === 'manager' ? '010-1234-5678' : '010-9876-5432'}
          />
        </section>
        <aside className="panel detail-aside">
          <h2>{role === 'manager' ? '관리 건물' : '내 거주지'}</h2>
          <InfoRow label="건물" value="A타워" />
          <InfoRow
            label={role === 'manager' ? '주소' : '호실'}
            value={role === 'manager' ? '서울 강남구 역삼동 123-4' : '302호'}
          />
          {role === 'resident' && (
