...
export function ManagerProfilePage() {
  ...
  const currentUserName =
    auth.status === 'authenticated' ? auth.user.userName : null;
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .then((user) => {
        ...
        setMarketingAgreed(
          user.agreements?.find((item) => item.termsType === 'MARKETING')
            ?.isAgreed ?? false,
        );
      })
      .catch(() => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await auth.updateManagerProfile(
        buildManagerProfileRequest({ userName: name, phone, marketingAgreed }),
      );
    ...
    <main className="focused-flow">
      ...
      <form className="flow-card manager-profile-card" onSubmit={submit}>
        <button
          ...
          onClick={() => navigate(-1)}
        >
          <IconArrowLeftLine /> 관리자 가입
        </button>
        <div>
          <p className="eyebrow">관리자 가입 · 프로필</p>
          <h1>프로필을 확인해 주세요</h1>
          <p>건물 관리를 위해 이름과 연락처를 확인해요.</p>
        </div>
        <TextField
          label="이름"
          showRequiredIndicator
          required
          description="최대 7자까지 입력할 수 있어요."
        >
          <TextFieldInput
            value={name}
            maxLength={7}
        ...
        <TextField
          ...
          showRequiredIndicator
          required
          description="숫자 10~11자리를 입력해 주세요."
        >
          <TextFieldInput
            value={phone}
            maxLength={13}
...
function profileErrorMessage(error: unknown) {
  ...
  if (error.status === 400 || error.status === 422)
    return '이름과 전화번호를 확인해 주세요.';
  return (
    error.message || '프로필을 저장하지 못했어요. 잠시 후 다시 시도해 주세요.'
  );
}
