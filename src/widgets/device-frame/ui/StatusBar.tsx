...
export function StatusBar({ device }: { device: PreviewDevice }) {
  ...
  const time = device === "iphone" ? `${hours % 12 || 12}:${minutes}` : `${String(hours).padStart(2, "0")}:${minutes}`;
  const batteryPercent = battery ? Math.round(battery.level * 100) : null;
  const batteryLabel = batteryPercent === null ? "배터리 정보 없음" : `배터리 ${batteryPercent}%${battery?.charging ? ", 충전 중" : ""}`;

  return (
    <div className="device-status-bar" role="status" aria-label={`${time}, ${online ? "온라인" : "오프라인"}, ${batteryLabel}`}>
      <time className="device-status-time" dateTime={now.toISOString()} aria-hidden="true">{time}</time>
      <span className="device-camera" aria-hidden="true" />
      <span className="device-status-icons" aria-hidden="true">
        <SignalIcon />
        <WifiIcon online={online} />
        {device === "android" && batteryPercent !== null && <span className="device-battery-percent">{batteryPercent}%</span>}
