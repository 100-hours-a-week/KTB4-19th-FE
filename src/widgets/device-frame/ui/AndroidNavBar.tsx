...
export function AndroidNavBar() {
  ...
  return (
    <nav className="device-nav-bar" aria-label="기기 내비게이션">
      ...
      <button type="button" className="device-nav-button" aria-label="홈" onClick={() => navigate("/")}>
        <span className="device-nav-home" aria-hidden="true" />
      </button>
      <button type="button" className="device-nav-button" aria-label="뒤로" onClick={() => navigate(-1)}>
